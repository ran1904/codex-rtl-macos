...
assert.match(runtime, /const UI_TEXT = 'span, label, button, summary'/);
assert.match(runtime, /hasDirectText/);
assert.match(runtime, /isInlineRichText/);
assert.match(runtime, /\[data-markdown-copy="inline-code"\]/);
assert.match(runtime, /const RESPONSE = '\[data-response-annotation-target\]'/);
assert.match(runtime, /responseFor/);
assert.match(runtime, /promptDirectionFor/);
assert.match(runtime, /OUTPUT_DIRECTION_THRESHOLD/);
assert.match(runtime, /localCodexRtlResponse/);
assert.match(css, /unicode-bidi: isolate/);
assert.match(css, /local-codex-rtl-response/);
